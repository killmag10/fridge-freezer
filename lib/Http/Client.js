...
Client.prototype.bufferSize = 65536;
...
var request = function(client, params) {
    ...
    if (bodyStream !== null) {
        ...
        response.body = [];
        while (
            (readedLength = bodyStream.read(buffer, 0, client.bufferSize)) > 0
        ) {
            var jsBuffer = new Buffer(buffer.length);
            for (var i = 0; i < buffer.length; i++) {
                jsBuffer.writeInt8(buffer[i], i);
            }

            response.body.push(jsBuffer);
        }
    ...
    if (params.closeIdleConnections) {
