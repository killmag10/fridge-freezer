...
var Structure = function(options)
{
    ...
    var database = new JavaDatabase(_step_.getTransMeta().findDatabase(options.database));
    database.connect();
    ...
    {
        var sql =
            'SELECT id FROM `' + options.changelogName + '` WHERE appliedAt IS NULL LIMIT 1';
    ...
    var getStructureChanges = function()
    {
        ...
        var changes = files.filter(
            ...
            function(file) {
                ...
                var forwardSql = fs.readFileSync(forwardPath);
                var description = fs.readFileSync(descriptionPath);
    ...
    this.update = function()
    {
        options.logger && options.logger.info(
            '----- BEGIN DATABASE STRUCTURE UPDATE -----'
        );
        validateChangelog();
        ...
        options.logger && options.logger.info(
            '----- FINISH DATABASE STRUCTURE UPDATE -----'
        );
    }
