...
var QueryFactory = function(options)
{
    ...
    this.getUpdateOld = function getUpdateOld()
    {
        return [
            ...
            'WHERE ',
            fieldsToString(this.options.fieldDescriptions.getFieldsByType(OptionFieldTypes.id), ' AND '),
            'AND ( ' + escapefield(this.options.fieldNames.dateTo) + ' >= NOW() OR ',
            escapefield(this.options.fieldNames.dateTo) + ' IS NULL )'
        ].join('\n');
    };
    ...
    this.getPurgeOne = function getUpdateLastSeen()
    {
        return [
            'UPDATE '  + getTablePath() + " SET ",
            escapefield(this.options.fieldNames.dateTo) + ' = NOW()',
            'WHERE ',
            fieldsToString(this.options.fieldDescriptions.getFieldsByType(OptionFieldTypes.id), ' AND '),
            'AND ( ' + escapefield(this.options.fieldNames.dateTo) + ' >= NOW() OR ',
            escapefield(this.options.fieldNames.dateTo) + ' IS NULL )'
    ...
    this.getPurgeScope = function getPurgeScope()
    {
        var sql = [
            'UPDATE ' + getTablePath(),
            ' SET ' + escapefield(this.options.fieldNames.dateTo) + ' = NOW()',
            ' WHERE ( ' + escapefield(this.options.fieldNames.dateTo) + ' >= NOW() OR ',
            escapefield(this.options.fieldNames.dateTo) + ' IS NULL )',
